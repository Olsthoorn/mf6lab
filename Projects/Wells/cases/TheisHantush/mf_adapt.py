# Wells, Theis and Hantush
"""
An axially symmetric model is setup to compute the drawdown by wells, both Theis and Hantush type as well as any
multi-layer variants, such that all cases like delayed yield  and well bore storage can be delt with by setting the parameters appropriately.
...
Gwfsto = {'sy': gr.const(lay['Sy'].values, axial=True),
          'ss': gr.const(lay['Ss'].values, axial=True),
          'iconvert': gr.const(lay['ICELLTYPE'].values),
          }

# === Gwfnpf ===== Horizontal and vertical conductivity

Gwfnpf = {  'k':   gr.const(lay['k'].values,   axial=True),
            'k33': gr.const(lay['k33'].values, axial=True),
