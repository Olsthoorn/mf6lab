...
psi_levels = get_contour_levels(-P, P, 100)
hmin, hmax = np.unique(headsObj.get_alldata())[[0, -1]]
h_levels = get_contour_levels(hmin, hmax, n=50)

# === Flow between successive stream lines =====
dpsi = np.diff(psi_levels)[0]

Q = budObj.get_data(text='wel', kstpkper=kstpkper[-1])[0]['q'].sum()

# === Plot the cross section =====
title = "{} Q extracted = {:.1f} m3/d, Q desired = {:.1f} m3/d. Tussen 2 stroomlijnen = {:.2f} m3/d".format(
                                        mf_adapt.section_name, Q, pr['Q'], dpsi)

ax = newfig(title, 'distance r [m]', 'elevation [m]', xscale='linear', xlim=(0, 200), figsize=(15, 8))

# === plot the water table =====
hwt = np.array([h[iz, 0, ix] for ix, iz in enumerate(Iz)]) # Water table
ax.plot(gr.xm, hwt, 'b-', lw=1, label='water tafel') # plot water table

for ilay, hLayer in enumerate(headsObj.get_data(kstpkper=kstpkper[-1])[:, 0, :]):
    ax.plot(gr.xm, hLayer, label='ilay = {}'.format(ilay))

# === contour the heads =====
hM = headsObj.get_data(kstpkper=kstpkper[-1])
hZ = gr.heads_at_Z(hM, fflows['frf'][-1], gr.const(mf_adapt.lay['k33'].values))
...
ax.contour(gr.Xp, Zpx, psi, levels=psi_levels, lw=0.5, label='Psi')
