...
# Verificatie van het module

def checkFlows(headsObj=None, budgetObj=None, lay=None, istep=None, grb_file=None):
    
    kstpkper = headsObj.get_kstpkper()
    ksp = kstpkper[istep]
    
    fflows = get_structured_flows_as_dict(budgetObj, grb_file)
    
    FLF = fflows[ksp]['flf'][0]
    FFF = fflows[ksp]['fff'][0]
    FRF = fflows[ksp]['frf'][0]
    
    assert np.all(FLF[-1].ravel() == 0.), "All flf[-1] must be zero! Sum is {} m3/d".format(FLF[-1].sum())
    
    qvMF6 = FLF[:-1] / gr.Area[np.newaxis, : ,:]
    ...
    print("mean qvMF6 ={}, mean qvHand ={}".format(qvMF6.mean(), qvHand.mean()))
    print()
    
def checkHeadComputation(headsObj=None, budgetObj=None, lay=None, istep=None, grb_file=None):
    kstpkper = headsObj.get_kstpkper()
    ksp = kstpkper[istep]
    ...
    if True:
        htop = hds + (3 * qtop + 1 * qbot) * cLay / 8
        hbot = hds - (1 * qtop + 3 * qbot) * cLay / 8
    else:
        htop = hds + qtop * cLay / 2
        hbot = hds - qbot * cLay / 2
    
    hcompare = np.vstack((htop[:, 0, :].mean(axis=-1), hbot[:, 0, :].mean(axis=-1), hds[:, 0, :].mean(axis=-1)))
    
    # Bereken de heads in de andere laag via de qv!
    
    print("mean htop ={}\n mean hbot ={}\nmean hds = {}".format(htop[:, 0, :].mean(axis=-1),
                                                               hbot[:, 0, :].mean(axis=-1),
                                                               hds[ :, 0, :].mean(axis=-1))
          )
...
checkFlows(headsObj=headsObj, budgetObj=budObj, lay=lay, istep=5, grb_file=grb_file)
checkHeadComputation(headsObj=headsObj, budgetObj=budObj, lay=lay, istep=5, grb_file=grb_file)
