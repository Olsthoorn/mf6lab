...
IActive = gr.inactive_cells_array(IDOMAIN)
...
sim_times = np.array(concObj.times)

title = "{}, rch = {:.1f} mm/d. dPsi = {:.2f} m3/d, rhoSalt/rhoFresh = {:.0f} kg/m3".format(
    mf_adapt.section_name, pr['rch'] * 1000, dpsi, pr['rhoSalt'] / pr['rhoFresh'])
...
c.ravel()[IActive['inact']] = 0.5 * (c.ravel()[IActive['itop']] + c.ravel()[IActive['ibot']])
...
def update(frame, headsObj=None, concObj=None, fflows=None, kstpkper=None, sim_times=None, ax=None, cmap=None, pLevels=None, cLevels=None, IActive=None):
    """Animation update function."""
    global ttl, wt, caxP, caxC

    # Update title
    ttl.set_text(title + ' frame={}, time={} d'.format(frame, sim_times[frame]))
