...
defaults = {'d_drain': 0,  # [m] Tile drainage depth below local ground elevation, may be zero if no drains.
            ...
            'wi_ditch2' : 2.,  # [d] Ditch resistance regional aquifer when flow is from ditch to ground. (analytical)
            'wo_ditch' : 1.,  # [d] Ditch resistance when flow is from ground to ditch. (anlaytical)
            'wo_ditch2' : 1.,  # [d] Ditch resistance regional aquifer when flow is from ground to ditch. (anlaytical)
...
# For legend when plotting the running water budget
...
class Dir_struct:
    ...
    def exe_name(self):
        ...
        if os.name == 'posix':
            exe_path = os.path.join(self.bin, 'mf2005.mac') # Version 2021 mf6.2.1. Compiled on Mac using pymake.
        else:
            exe_path = os.path.join(self.bin, 'mf2005.exe') # Version 2021 mf6.2.1
        return exe_path
...
def grid_from_parcel_data(parcel_data=None, dx=None, laycbd=(1, 0)):
    ...
    parcel_data: pd.DataFrame
        table of parcel property data
    dx: float
        column width (columns all have the same width in GGOR, e.g. 1 m)
    LAYCBD: list (or sequence) of length 2 (nlayer)
        value of 1 for each layer with a confining unit below it, else value of 0
    """
    bmax = parcel_data['b'].max() # base model grid width on widest parcel in the database
    nx = int(np.ceil(bmax) // dx)
    ny = len(parcel_data)
    nz = 2 # GGOR always had a cover layer and a regional layer with a  confining bed in between.
...
def set_spatial_arrays(parcel_data=None, gr=None):
    """Create the spatial arrays for MODFLOW.
    ...
    # Because
    sparr['laytyp']=0
    # SY is not used (but generated in case we want to use a varying top-layer thickness)
    ...
    # IBOUND:
    # Limit width of rows to width of parcels by making cells beyond each parcel's width inactive
...
def get_parcel_average_hds(HDS=None, IBOUND=None, gr=None):
    ...
    """
    #TODO: Check en vereenvoudig de procedure die is nu niet zomaar te doorzien.
...
def run_modflow(dirs=None, parcel_data=None, tdata=None, laycbd=(1, 0), dx=1.,
        ...
        pcg = fm.ModflowPcg(mf, mxiter=200, iter1=200, hclose=hclose, rclose=rclose)
        #sms = fm.ModflowSms(mf, mxiter=mxiter, iter1=iter1, hclose=hclose, rclose=rclose)

        packages = {'dis': dis, 'bas':bas, 'lpf':lpf, 'ghb':ghb, 'riv':riv, 'drn':drn,
                    'wel':wel, 'rch':rch, 'evt':evt, 'oc':oc, 'pcg': pcg}  # , 'sms':sms, 'pcg':pcg, '
