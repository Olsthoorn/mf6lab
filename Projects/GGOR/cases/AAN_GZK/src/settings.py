...
HOME = '/Users/Theo/GRWMODELS/python/mf6lab/Projects/GGOR/cases/AAN_GZK'
assert os.path.isdir(HOME), f"No directory {HOME}"
os.chdir(HOME)

case_name = Path(HOME).parts[-1]

# Generate the name space for the directories of this project
dirs = Dirs()
dirs.meteo = os.path.join(dirs.data, 'meteo')
dirs.bofek = os.path.join(dirs.data, 'bofek')
