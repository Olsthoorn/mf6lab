...
# We'll try to set up an areal model based on an earlier cross section through the Drasse Driehoek area, Tilburg.
# The layers will be the same as in the cross section, but the model will be areal.
# The model will be orientated West East for convenience.
# We must set the Model's bounding box and grid cell size, which will be squares of 25x25 m.
# The need ground surface elevation according to AHN model.
# We then need aquifer properties.
# We need boundary conditions.
# We need recharge.

# First try to get the layers from REGIS of NHI model
