...
NOTICE:
------- 
This version uses the IDOMAIN == -1 (flow thorugh cells) option in MF6 where layers are pinched out (don't exist).
These IDOMAIN == -1 cells are inactive in MF6 and get values 1e30 for heads and concentrations.
Especially for concentrations, this hinders contouring. The problem is solved by first seeking
the first active cell above the inactive cell and then the first active cell below the inactive cell,
after which the value in the inactive cell can be interpolated from these upper and lower active cells.
