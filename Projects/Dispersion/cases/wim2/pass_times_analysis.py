...
# Raw break th
fig, ax = plt.subplots(figsize=(10, 6))
ax.set(title='BTCs normalized with t_mean (case ' + case_name + ') with the folowing k-field parameters:\n' + k_field_str, xlabel='tau (t / tmean)', ylabel='p[t < t[p]]')
p = pass_times['id'] / 1000.
for X, t in zip(cumul_t['xObs'].T, cumul_t['time'].T):
    ax.plot(t / np.mean(t), p, label=f'x = {X[0]} m')
    #ax.plot(t, p, label=f'x = {X[0]} m')
...
x_last = last_xvals[int(0.95 * len(last_xvals))]
...
for it, (T, x) in enumerate(zip(cumul_x['tObs'].T, cumul_x['x'].T)):
    ...
    par_log_x = stats.lognorm.fit(x[x < xE])    
    cdf_log_x = stats.lognorm.cdf(x[x < xE], *par_log_x)
    pdf_log_x = stats.lognorm.pdf(x[x < xE], *par_log_x)
    ...
    if t in cumul_x['tObs'][0][::2]:
        ...
        ax1.plot(x[x < xE], cdf_log_x, color=clr,
                 label=fr"t={t:.0f} d, $\mu,\sigma,\,t_0$,loc=({mu_x:.2f}, {sig_x:.2f}, {loc_x:.3e} m)")
        ax2.plot(x[x < xE], pdf_log_x, color=clr, 
